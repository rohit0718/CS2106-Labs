...
void release_table(int table_id) {
...
int has_waiting(int table_id) {
    ...
    // for ex5, we can assign table to earliest grp in queue
    // whose sz <= table_sz
...
table_t* has_table(int num_ppl) {
    // for ex5, grp with num_ppl can be assigned to table sz >= num_ppl
    // we prioritize the table with smallest num of seats
