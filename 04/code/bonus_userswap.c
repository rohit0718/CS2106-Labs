...
// linked list contains starting byte number
...
// if file map & dirty, write back changes to file
void userswap_free(void *mem) {
  ...
  while (head) {
    ...
    if (head->info->start == mem) {
      ...
      pthread_mutex_lock(&lock); // lock all updates to linked list and virtual mem
